...
def get_args_parser():
    ...
    # StK: map the 90 classes from COCO to the 3 classes of CARLA:
    parser.add_argument('--coco_re_mapped_class', action='store_true', default=False,
                        help='Mapped the classes of COCO from 90 classes to 3 classes in CARLA. \
                              Remember to modify the __init__.py file in datasets/torchvision_datasets/.')
    # StK: we add a batch size for validation:
    parser.add_argument('--val_batch_size', default=1, type=int,
                        help='This is the batch size for validation loader. The postprocess we modified only support  \
                        batch size of 1. Therefore, we will only use value of 1 here.')

    # StK: add a our own version for resume from a coco pretrain checkpoint
...
def main(args):
    ...
    # StK: we add the way for loading pretrain COCO Deformable DETR model
    if args.coco_pretrain_ckpt is not None:
        checkpoint = torch.load(args.coco_pretrain_ckpt, map_location='cpu')
        model_without_ddp.load_state_dict(checkpoint["model"])
        #add_1_from_detr = True # if we choose to add 1 as DETR setup
        add_1_from_detr = False
        model_without_ddp.update_class_embed(new_num_classes=4, device=device, 
                                             add_1_from_detr=add_1_from_detr, 
